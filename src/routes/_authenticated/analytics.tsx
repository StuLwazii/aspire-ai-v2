...
import { BarChart3, Sparkles, Download, FileText, Trash2, Clock, Calendar, Plus, Eye, Loader2 } from "lucide-react";
import {
...
export const Route = createFileRoute("/_authenticated/analytics")({
  ...
  head: () => ({ meta: [{ title: "Analytics & Reports — Aspire AI" }] }),
});
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto">
      <header>
        ...
        <p className="text-muted-foreground mt-1">KPIs, trends, and AI-generated executive reports across all tickets.</p>
      </header>

      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList>
          <TabsTrigger value="overview"><BarChart3 className="h-4 w-4 mr-1.5" /> Overview</TabsTrigger>
          <TabsTrigger value="generate"><Sparkles className="h-4 w-4 mr-1.5" /> Generate report</TabsTrigger>
          <TabsTrigger value="history"><FileText className="h-4 w-4 mr-1.5" /> Report history</TabsTrigger>
        ...
        <TabsContent value="generate">
          <Card className="p-5 space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              The AI reads live dashboard data, computes KPIs (volume, resolution rate, response times, trend vs previous period), and writes an executive-level summary, performance analysis, and recommendations. The report is archived under Report history.
            </p>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="history" className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {!reportsQ.isLoading && filteredReports.length === 0 && (
              <Card className="p-8 text-center text-sm text-muted-foreground">No reports yet. Generate your first one from the Generate report tab.</Card>
            )}
